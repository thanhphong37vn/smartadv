...
if (isset($_GET['page']) && (int) ($_GET['page'])) {
    ...
    $query = "SELECT COUNT(MaSP) FROM tbl_sanpham WHERE MaLoai = {$id}";
...
$sql = "SELECT A . * , B.DuongDan, C.TenNhaSX FROM tbl_sanpham A INNER JOIN tbl_hinhanh B INNER JOIN tbl_nhasx C ON C.MaNhaSX = A.MaNhaSX AND A.MaSP = B.MaSP WHERE MaLoai = {$id}  LIMIT $start,$display";
    ...
    while ($set = mysql_fetch_array($result, MYSQLI_ASSOC)) {
        echo "<div class='topproduct'>
                <div class='topproduct2'>
                    <div class='image'><a href='index.php?page=product_detail&id={$set['MaSP']}&ml={$set['MaLoai']}'><img src='{$set['DuongDan']}' width='160' height='155'/></a></div>
                    <div class='titlep'><a href='index.php?page=product_detail&id={$set['MaSP']}'>{$set['TenSP']}</a></div>
