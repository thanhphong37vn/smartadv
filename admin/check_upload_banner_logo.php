<?php
function check_upload_banner_logo() {
    $file = null;
    ...
    if (in_array($duoiFileAnh, $ext)) {
        if ($_FILES["uploadanhchinh"]["error"] > 0) {
            ...
            $no = 1;
            while (true) {
                if (file_exists("../images_ads/" . $fileName)) {
                    if ($flag == 0) {
                        $fileGoc = $fileFinfo[0];
                        $flag = 1;
                    }
                    ...
                    move_uploaded_file($_FILES["uploadanhchinh"]["tmp_name"],"../images_ads/".$fileName);
                    $file = "images_ads/" . $fileName;
                    break;
                }
            }
        }
